...
describe("applyLoaders", function() {
  ...
  describe('pitching loaders', function() {
    ...
    beforeEach(function(done) {
      ...
      const loaders = [
        ...
        TestUtils.createLoaderFromString(multiline(function() {
          ...
          //   global.pitchLoaderSpecState.dataMap['c'] = this.data;
          //
          //   return s;
          // };
          //
        ...
        TestUtils.createLoaderFromString(multiline(function() {
          ...
          //
          //   return '// intercepted by b loader pitch';
          // };
        })),
    ...
    it('applies #pitch from left-to-right', function() {
      assert.deepEqual(state.pitchIndex, [ 'c', 'b']);
    });

    it('ignores loaders to the right if a pitch yields', function() {
      assert.deepEqual(state.applyIndex, [ 'b', 'c', 'd' ]);
    });

    it('passes the correct remainingRequest fragment', function() {
      assert.equal(state.remainingRequestMap.c, "b!a!./foo.js");
      assert.equal(state.remainingRequestMap.b, "a!./foo.js");
    });

    it('passes the correct precedingRequest fragment', function() {
      assert.equal(state.precedingRequestMap.c, "d");
      assert.equal(state.precedingRequestMap.b, "d!c");
    });

    it('passes @data between pitch and normal phase', function() {
      assert.deepEqual(Object.keys(state.dataMap), [ 'c' ]);
      assert.deepEqual(state.dataMap.c, { foo: 'bar' });
    });

    it('uses the result yielded by any pitch loader', function() {
      assert.equal(state.code, '// intercepted by b loader pitch');
    });
  });
});
