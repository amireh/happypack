  ...
  async.series([
    function normalizeLoaders(callback) {
      ...
      assert(loaders && loaders.length > 0,
        ...
        "and there is no matching loader entry with this id."
      );
...
HappyPlugin.prototype.stop = function() {
  assert(this.state.started, "HappyPlugin cannot be stopped until it is started!");
...
function compileInBackground(loader, loaderContext, done) {
  ...
  if (this.state.debug) {
    console.warn('File has changed; re-compiling... (%s)', filePath);
  }

  compile.call(this, this.threadPool, loader, loaderContext, done);
}
...
function compileInForeground(loader, loaderContext, done) {
  compile.call(this, this.state.foregroundThreadPool, loader, loaderContext, done);
}

function compile(threadPool, loader, loaderContext, done) {
