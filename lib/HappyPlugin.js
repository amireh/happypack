  ...
  this.config = OptionParser(userConfig, {
    ...
    loaders: {
      validate: function(value) {
        // if (userConfig.id && userConfig.inferLoaders) {
        //   return;
        // }
        // else
      ...
      isRequired: true,
      // isRequired: function() {
      //   return !userConfig.id || !userConfig.inferLoaders;
      // }
    }
  }, this.id);
...
HappyPlugin.prototype.start = function(compiler, done) {
  ...
  async.series([
    // function inferLoaders(callback) {
    //   if (!that.config.inferLoaders) {
    //     return callback();
    //   }

    //   var set = HappyPlugin.extractLoaders(that.id, compiler.options.module.loaders);
    //   assert(set.happyLoaders);


    //   that.config.loaders = set.happyLoaders;
    //   compiler.options.module.loaders = set.webpackLoaders;
...
HappyPlugin.prototype.compileInForeground = function(source, map, filePath, done) {
  ...
  WebpackUtils.applyLoaders(options, source, map, filePath, function(err, result) {
    if (err) {
      return done(err);
    }
...
// HappyPlugin.extractLoaders = function(id, entries) {
//   var happyLoaders;
//   var webpackLoaders = entries.map(function(entry) {
//     if (!entry.happy || entry.happy.enabled === false || entry.happy.id !== id) {
//       return entry;
//     }
//     else if (entry.happy.id === id && happyLoaders) {
//       assert(false,
//         "You have more than one loader referencing the same HappyPlugin with the id '" + id + "'!"
//       );
//     }

//     if (entry.loaders) {
//       happyLoaders = entry.loaders;
//     }
//     else if (entry.loader) {
//       happyLoaders = [ entry.loader ];
//     }

//     // var moddedEntry = Object.create(entry);
//     var moddedEntry = entry;


//     entry.loader = path.resolve(__dirname, 'HappyLoader.js') + '?id=' + id;
//     delete moddedEntry.loaders;

//     return moddedEntry;
//   });

//   return {
//     happyLoaders: happyLoaders,
