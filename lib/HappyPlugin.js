...
HappyPlugin.prototype.apply = function(compiler) {
  ...
  engageWatchMode = fnOnce(function() {
    ...
    WebpackUtils.tapInto(compiler, 'done', false, function() {
  ...
  WebpackUtils.tapInto(compiler, 'watch-run', true, function(_, done) {
  ...
  WebpackUtils.tapInto(compiler, 'run', true, that.start.bind(that));

  // cleanup hooks:
  WebpackUtils.tapInto(compiler, 'done', false, that.stop.bind(that));

  if (compiler.options.bail) {
    WebpackUtils.tapInto(compiler, 'compilation', false, function(compilation) {
      WebpackUtils.tapInto(compilation, 'failed-module', false, that.stop.bind(that));
    });
  }
};
