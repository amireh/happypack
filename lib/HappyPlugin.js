...
HappyPlugin.prototype.apply = function(compiler) {
  ...
  var that = this;
  var watchModeEngaged = false;

  compiler.plugin('watch-run', function(watching, done) {
    if (watchModeEngaged) {
      that.state.foregroundWorker._compiler.applyPlugin('watch-run', {
        watching: watching
      }, done);
    }
    else {
      watchModeEngaged = true;
      ...
      compiler.plugin('done', fnOnce(function() {
        ...
        compiler.plugin('after-compile', function(compilation, callback) {
          ...
          that.state.foregroundWorker._compiler.applyPlugin('after-compile', {
