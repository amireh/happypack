...
HappyPlugin.prototype.apply = function(compiler) {
  ...
  compiler.plugin('watch-run', function(watching, done) {
    var pluginDone = function(err) {
      if (err) {
        return done(err);
      }

      that.threadPool.executePlugin('watch-run', done, {
        watching: {}
      });
    };

    if (engageWatchMode() === fnOnce.ALREADY_CALLED) {
      that.resetSharedState();
      pluginDone();
    }
    else {
      that.start(compiler, pluginDone);
    }
  });
  ...
  compiler.plugin('compilation', function(compilation) {
    HappyRPCHandler.registerActiveCompilation(compilation);
  ...
  compiler.plugin('after-compile', function(compilation, callback) {
    that.threadPool.executePlugin('after-compile', callback);
  });
};
...
HappyPlugin.prototype.resetSharedState = function() {
