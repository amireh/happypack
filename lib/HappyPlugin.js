...
HappyPlugin.prototype.apply = function(compiler) {
  var that, webpack4PluginSupport, engageWatchMode, watchRunCallback, doneCallback;

  that = this;
  webpack4PluginSupport = 'hooks' in compiler;
  ...
  engageWatchMode = fnOnce(function() {
    if (webpack4PluginSupport) {
      compiler.hooks.done.tap('HappyPlugin', doneCallback);
    } else {
      compiler.plugin('done', doneCallback);
    }
  });

  watchRunCallback = function(_, done) {
  ...
  if (webpack4PluginSupport) {
    compiler.hooks.watchRun.tapAsync('HappyPlugin', watchRunCallback);

    compiler.hooks.run.tapAsync('HappyPlugin', that.start.bind(that));

    // cleanup hooks:
    compiler.hooks.done.tap('HappyPlugin', that.stop.bind(that));
  } else {
  ...
  if (compiler.options.bail) {
    if (webpack4PluginSupport) {
      compiler.hooks.compilation.tap('HappyPlugin', function(compilation) {
        compilation.hooks.failedModule.tap('HappyPlugin', that.stop.bind(that));
      });
    } else {
...
HappyPlugin.prototype.start = function(compiler, done) {
  ...
  async.series([
    function resolveLoaders(callback) {
      var loaderSpecs = that.config.rules || that.config.loaders;
