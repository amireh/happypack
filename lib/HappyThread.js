  ...
  return {
    open: function(onReady) {
      ...
      fd.on('message', function acceptMessageFromWorker(message) {
        ...
        else if (message.name === 'EXECUTE_PLUGIN_DONE') {
          callbacks['EXECUTE_PLUGIN:' + message.data.phase](message.data.error);
        }
      });
    },
    ...
    executePlugin: function(phase, payload, done) {
      callbacks['EXECUTE_PLUGIN:' + phase] = done;
