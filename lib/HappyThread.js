  ...
  return {
    open: function(onReady) {
      ...
      fd.on('message', function acceptMessageFromWorker(message) {
        ...
        else if (message.name === 'APPLY_PLUGIN_DONE') {
          callbacks['APPLY_PLUGIN:' + message.data.phase](message.data.error);
        }
      });
    },
    ...
    applyPluginInBackground: function(phase, payload, done) {
      callbacks['APPLY_PLUGIN:' + phase] = done;
