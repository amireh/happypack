...
module.exports = function HappyThreadPool(config) {
  ...
  return {
    ...
    executePlugin: function(phase, done, args) {
      if (state.executedPlugins.indexOf(phase) > -1) {
        return done();
      }
      ...
      async.series(threads.filter(send('isOpen')).map(function(thread) {
        return async.apply(thread.executePlugin, phase, payload);
      }), function(rawError) {
        done(ErrorSerializer.deserialize(rawError));
      });
    },
