...
var webpackVersion = parseInt(require('webpack/package.json').version.match(/(^\d)\b/)[0], 10);
...
exports.extractLoaders = function extractLoaders(entry) {
  if (typeof entry === 'object' && entry.loaders) {
    ...
    return entry.rules.reduce(function(list, loader) {
      return list.concat(extractLoaders(loader));
    }, []);
  }
...
exports.tapInto = function tapInto(target, hook, isAsyncHook, callback) {
  // webpack 4 plugin
  if('hooks' in target) {
